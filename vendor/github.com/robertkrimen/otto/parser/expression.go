...
func (self *_parser) parsePrimaryExpression() ast.Expression {
    ...
    case token.RIGHT_PARENTHESIS:
		self.next()
		if self.token == token.ARROW_POINTER {
			return self.parseArrowFunction(&ast.ParameterList{})
		}
		break
	case token.LEFT_PARENTHESIS:
		...
		_, ok := expression.(*ast.FunctionLiteral)
		if !ok {
...
func (self *_parser) parseVariableDeclarationList(var_ file.Idx) []ast.Expression {
	...
	for {
		...
		if self.token == token.ARROW_POINTER {
			var list = (decl.(*ast.VariableExpression))
			list.Initializer = self.parseArrowFunction(self.getArrowArguments(list.Initializer))
		}
...
func (self *_parser) getArrowArguments(exp ast.Expression) *ast.ParameterList {
	var l = ast.ParameterList{}
	var castedList, ok = exp.(*ast.SequenceExpression)
	if ok {
		for _, e := range castedList.Sequence {
			var casted = e.(*ast.Identifier)
			l.List = append(l.List, &ast.Identifier{
				Name: casted.Name,
				Idx: casted.Idx,
			})
		}
	} else {
    	var castedEntry, ok = exp.(*ast.Identifier)
    	if ok {
			l.List = append(l.List, castedEntry)
    	}
	}
	return &l
}
func (self *_parser) parseArgumentList() (argumentList []ast.Expression, idx0, idx1 file.Idx) {
	...
	if self.token != token.RIGHT_PARENTHESIS {
		for {
			...
			if self.token == token.ARROW_POINTER {				
				exp = self.parseArrowFunction(self.getArrowArguments(exp))
			}
