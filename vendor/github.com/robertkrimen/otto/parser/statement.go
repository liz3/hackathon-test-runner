...
func (self *_parser) parseStatement() ast.Statement {
	...
	case token.CONST:
		return self.parseVariableStatement(token.CONST)
	case token.LET:
...
func (self *_parser) parseArrowFunction(paramList *ast.ParameterList) *ast.FunctionLiteral {
	node := &ast.FunctionLiteral{
		Function: self.expect(token.ARROW_POINTER),
	}
...
func (self *_parser) parseVariableStatement(tkn token.Token) *ast.VariableStatement {
	var comments []*ast.Comment
	if self.mode&StoreComments != 0 {
		comments = self.comments.FetchAll()
	}
